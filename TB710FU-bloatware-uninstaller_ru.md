...
# 🧹 Удаление мусорных приложений, отключение OTA-обновлений и фикс локалей в TB710FU
...
---
Ссылка для скачивания сейчас будет
---

> ⚠️ **ОТКАЗ ОТ ОТВЕТСТВЕННОСТИ: ВСЕ ДЕЙСТВИЯ НА СВОЙ СТРАХ И РИСК!**  
> Инструмент удаляет системные приложения через `pm uninstall --user 0`. Приложения удаляются **только для текущего пользователя** и могут быть восстановлены через ADB. Однако удаление некоторых пакетов может повлиять на стабильность системы. **Всегда просматривайте список пакетов перед запуском скрипта.**
...
- 🌎 **Фикс для служб Google и RU-локали (CN-скрипт):** CN-скрипт автоматически устанавливает русскую локаль и снимает ограничения на фоновую работу приложений Google.
...
- 🎯 **Точечное воздействие:** Специально настроено для Lenovo Xiaoxin Pad Pro GT (TB710_ZUXOS_1.5.04.470)/ Yoga Tab 11.1 (TB710FU_ZUI_17.5.04.070).
...
1. **Windows 10 / 11**
2. **ADB-драйверы** должны быть установлены на ПК
...
1. Откройте **Настройки → О планшете → Версия Android**
2. Нажмите на **Версия ПО** 7 раз, пока не появится сообщение "Вы стали разработчиком!"
...
3. **Выберите язык** (CN-скрипт) или продолжите сразу (EN/RU скрипты).
...
| Проблема | Решение |
|----------|---------|
| Устройство не отображается в ADB | Включите отладку по USB на планшете. Проверьте встроенную справку (нажмите `Н` при запросе). |
